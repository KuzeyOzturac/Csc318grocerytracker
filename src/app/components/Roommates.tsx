...
import { ArrowLeft, Search, Heart, Lock } from "lucide-react";
...
export const Roommates = () => {
  ...
  const { roommates, user, addRoommate, removeRoommate } = useApp();
  ...
  const [searchResults, setSearchResults] = useState<Array<{
    ...
    username: string;
    initials: string;
    status: "available" | "already-roommate";
  }>>([]);
  ...
  const handleSearch = () => {
    ...
    const mockResults = [
      ...
      },
      {
        ...
        username: "@itsjane",
        initials: "JA",
        status: "already-roommate" as const,
      },
    ];

    setSearchResults(mockResults.filter(r => 
      r.username.toLowerCase().includes(searchUsername.toLowerCase())
    ));
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white border-b border-gray-200 p-6">
        ...
        <h2 className="text-xl font-medium text-gray-900 mb-4">Find Your Roommates</h2>
        ...
        <p className="text-sm text-gray-600 text-center mt-4">
          Search for your roommates by entering in their Username.<br />
          (i.e. @johnsmith)<br />
          <br />
        ...
        {searchResults.map((result, index) => (
          ...
          >
            <div className="flex items-start gap-4">
              ...
              {result.status === "already-roommate" && (
            ...
            {result.status === "already-roommate" && (
              <div className="mt-4 p-4 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-700">
                  <strong>IMPORTANT:</strong> It seems {result.username} already shares groceries with other roommates. 
                  You can send a request. If {result.username} accepts, their grocery list, as well as their roommates', 
                  will merge with you and your roommates' grocery list.
                </p>
              </div>
            )}
          </motion.div>
      ...
      <div className="p-6 pt-0">
        <h3 className="text-sm font-medium text-gray-600 mb-4">Current Roommates</h3>
        <div className="space-y-3">
          {allRoommates.map((roommate, index) => (
            ...
            >
              ...
              {roommate.id !== user.id && (
                <button
                  onClick={() => handleRemoveRoommate(roommate.id, roommate.name)}
